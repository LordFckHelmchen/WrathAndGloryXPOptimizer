...
def print_target_values_text(ctx, _, value) -> None:
    ...
    indent = "   "
    max_name_width = max(
        max(len(target_enum.name) for target_enum in target_enum_class.get_valid_members()) for target_enum_class in
    ...
    properties = ["The following target values are available:",
                  "TIER",
    ...
    for target_enum_class in [Attributes, Skills, Traits]:
        ...
        for target_enum in target_enum_class.get_valid_members():
            if not isinstance(target_enum, Traits):
                ...
                rating_bounds = IntBounds(target_enum.value.get_rating_bounds(related_tier=Tier.rating_bounds.min).min,
                                          target_enum.value.get_rating_bounds(related_tier=Tier.rating_bounds.max).max)
...
@click.version_option(version=f"{__version__} (Wrath & Glory core rules version "
                              f"{AttributeSkillOptimizer.WRATH_AND_GLORY_CORE_RULES_VERSION})")
@click.option("--help-target-values",
              is_flag=True,
