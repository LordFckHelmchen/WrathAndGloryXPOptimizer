...
class TestIntBounds(unittest.TestCase):
    def test_contains_with_values_within_bounds_expect_true(self) -> None:
    ...
    def test_contains_with_none_and_values_outside_bounds_expect_false(self) -> None:
        bounds = IntBounds(-8, 12)
        for val in [None, bounds.min - 1, bounds.max + 1]:  # type: ignore
            with self.subTest(i=val):
                self.assertFalse(
                    val in bounds,
                    f"Out-of-bound value was not identified: "
                    f"{val} not in {bounds}!",
                )

    def test_as_range_expect_bounds_to_be_met(self) -> None:
        min_val = -12
        max_val = 0
    ...
    def test_init_with_out_of_order_limits_expect_ordered_bounds(self) -> None:
        min_val = 7
        max_val = 100
    ...
    def test_init_with_none_in_bounds_expect_contains_to_be_always_false(self) -> None:
        min_val = None
        max_val = 666
    ...
    def test_init_with_none_in_bounds_expect_add_be_added_to_non_none_bound(
        self,
    ) -> None:
        min_val = 21
        max_val = None
    ...
    def test_add_int_expect_correctly_added_bounds(self) -> None:
        min_val = -56
        max_val = -2
        add_val = 5
    ...
    def test_add_none_expect_all_none_bounds(self) -> None:
        min_val = 752
        max_val = 98998
        add_val = None
    ...
    def test_add_float_expect_TypeError(self) -> None:
        with self.assertRaises(TypeError):
            _ = IntBounds(-1, 1) + 0.5  # type: ignore  # wrong type intended

    def test_add_IntBounds_expect_correctly_added_bounds(self) -> None:
        min_val = -7
        max_val = 236
        add_val = IntBounds(-3, 5)
        expected_bounds = IntBounds(min_val + add_val.min, max_val + add_val.max)  # type: ignore  # rating limits are always valid ints
        observed_bounds = IntBounds(min_val, max_val) + add_val
        self.assertEqual(expected_bounds, observed_bounds)

    def test_iter_expect_values_in_correct_order(self) -> None:
        bounds = IntBounds(-22, 9)
        self.assertEqual([bounds.min, bounds.max], [*bounds])

    def test_are_valid_expect_false_for_none_in_bounds(self) -> None:
...
class TestInvalidAttributeAndSkill(unittest.TestCase):
    def test_total_rating_bounds_expect_min_max_to_be_smaller_than_valid_min(
        self,
    ) -> None:
        for property_class, invalid_property in zip(
            [Attribute, Skill], [Attributes.INVALID.value, Skills.INVALID.value]
        ):
            for val in invalid_property.rating_bounds:  # type: ignore
                with self.subTest(i=f"{type(invalid_property).__name__}: {val}"):
                    self.assertFalse(property_class.is_valid_rating(val))  # type: ignore  # Derived class has function
                    self.assertIsNone(val)

    def test_is_valid_rating_expect_false(self) -> None:
        for property_class, invalid_property in zip(
            [Attribute, Skill], [Attributes.INVALID.value, Skills.INVALID.value]
        ):
            for val in [
                invalid_property.rating_bounds.min,  # type: ignore
                property_class.rating_bounds.max,  # type: ignore
            ]:
                with self.subTest(i=f"{type(invalid_property).__name__}: {val}"):
                    self.assertFalse(invalid_property.is_valid_rating(val))  # type: ignore


class TestInvalidSkill(unittest.TestCase):
    def test_total_rating_expect_same_invalid_value_as_rating(self) -> None:
        self.assertEqual(
            Skills.INVALID.value.rating_bounds, Skills.INVALID.value.total_rating_bounds
        )

    def test_is_valid_total_rating_expect_false(self) -> None:
