...
from wrath_and_glory_xp_optimizer.character_properties import Skills, Attributes, Traits
...
class TestPropertyResults(unittest.TestCase):
    ...
    def test_formatting_functions_expect_match_to_stored_format(self):
        ...
        result = AttributeSkillOptimizer(example_data["target_values"]).optimize_selection()
    ...
    def test_as_markdown_with_totals_below_equal_and_above_targets_expect_missed_only_for_below_target_values(self):
        expected_string = "Name           | Total  | Target | Missed\n" \
                          "-------------- | ------ | ------ | ------\n" \
                          "Fellowship     | 7      | 5      | NO    \n" \
                          "BallisticSkill | 4      | 4      | NO    \n" \
                          "MaxShock       | 9      | 10     | YES   "
        results = CharacterPropertyResults(Target={Attributes.Fellowship.name: 5,
                                                   Skills.BallisticSkill.name: 4,
                                                   Traits.MaxShock.name: 10},
                                           Total={Attributes.Fellowship.name: 7,
                                                  Skills.BallisticSkill.name: 4,
